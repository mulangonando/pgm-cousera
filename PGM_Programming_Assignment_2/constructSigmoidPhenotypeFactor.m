...
for k=1:length(phenotypeFactor.val)
    assign = IndexToAssignment(k, phenotypeFactor.card);
    z = 0;
%     for j=1:length(geneCopyVarOneList)
%         w = alleleWeights{geneCopyVarOneList(j)};
%         X = zeros(length(w),1);
%         Y = zeros(length(w),1);
%         X(assign(j+1)) = 1;
%         Y(assign(j+3)) = 1;
...
%     end;
    ...
    for gene=1:length(geneCopyVarOneList)
        X = zeros(length(geneCopyVarOneList), length(alleleWeights));
    Y = zeros(length(geneCopyVarOneList), length(alleleWeights));
       X((gene), assign(gene+1)) = 1;
       Y((gene), assign(gene+1+length(geneCopyVarOneList))) = 1;
       w = alleleWeights{gene};
       for all = 1:length(w)
           z = z+w(all)*(X(gene,all)+Y(gene,all));
       end
    end;
    if(assign(1) == 1)
    phenotypeFactor.val(k) = computeSigmoid(z);
    else
        phenotypeFactor.val(k) = 1-computeSigmoid(z);
    end;
end;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%